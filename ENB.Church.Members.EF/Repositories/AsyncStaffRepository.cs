...
namespace ENB.Church.Members.EF.Repositories
{
    ...
    public class AsyncStaffRepository : AsyncRepository<Staff>, IAsyncStaffRepository
    {
        ...
        public AsyncStaffRepository(ChurchMembersContext churchMembersContext) : base(churchMembersContext)
        {
            _churchMembersContext = churchMembersContext;
        }
        public IEnumerable<Staff> FindByName(string name)
        {
            return _churchMembersContext.Set<Staff>().Where(x => x.FirstName == name);
        }
    }
}
